...
class ChatMessageServiceTest {
  ...
  void getRecentMessages_limitExceedsMax_clampsTo200() {
    // Given
    when(chatMessagePort.findRecentByRoomId("ROOM_1", 200)).thenReturn(List.of());
  ...
  void getRecentMessages_limitZero_clampsTo1() {
    // Given
    when(chatMessagePort.findRecentByRoomId("ROOM_1", 1)).thenReturn(List.of());
  ...
  void getRecentMessages_returnsMessagesInAscendingOrder() {
    ...
    when(chatMessagePort.findRecentByRoomId("ROOM_1", 2)).thenReturn(descResult);
  ...
  void getRecentMessages_passesRoomIdToRepository() {
    // Given
    when(chatMessagePort.findRecentByRoomId("ROOM_XYZ", 10)).thenReturn(List.of());

    // When
    chatMessageService.getRecentMessages("ROOM_XYZ", 10);

    // Then
    verify(chatMessagePort).findRecentByRoomId("ROOM_XYZ", 10);
  }
